...
for line in readData:
    ...
    segments.append((tuple(map(int, end_1.split(','))),tuple(map(int,end_2.split(',')))))
...
for segment in segments:
    if segment[0][0] == segment[1][0]:
        min_col, max_col = min(segment[0][1], segment[1][1]), max(segment[0][1], segment[1][1])

        for col in range(min_col, max_col+1):
            diagram[segment[0][0]][col] += 1
    elif segment[0][1] == segment[1][1]:
        min_row, max_row = min(segment[0][0], segment[1][0]), max(segment[0][0], segment[1][0])
